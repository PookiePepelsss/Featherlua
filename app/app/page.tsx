...
export default function Home() {
  ...
  // What the chrome bar says, which is only ever something the app has
  // actually established: nothing has been sent anywhere, the compiler is
  // working, or it has accepted or rejected the output.
  const status = working
    ? { className: "chromeStatus statusWorking", text: "Validating with Luau" }
    : error
      ? { className: "chromeStatus statusError", text: "Error" }
      : output
        ? { className: "chromeStatus", text: "Verified with official Luau" }
        : { className: "chromeStatus", text: "Runs in your tab" };

  return (
    <main className="app">
      <div className="shell">
        <div className="shellGlow" aria-hidden="true" />
            ...
            <div className="results" aria-live="polite">
              <div className="resultMain">
                ...
                <span className="resultValue">
                  <span className="resultBytes">{stats.outputBytes.toLocaleString()}</span>
                  <span className="resultUnit">bytes</span>
                  {stats.inputBytes > 0 && (
                    <span className={stats.outputBytes <= stats.inputBytes ? "resultDelta" : "resultDelta resultGrew"}>
              ...
              <div className="resultMeta">
                <span className="resultNote">
                  {stats.inputBytes.toLocaleString()} B in · {stats.durationMs.toFixed(1)} ms
                </span>
                {/* How much of the original survived, so the bar shrinks as the
                    output does rather than filling as it grows. */}
                <div className="resultBar">
                  <span
                    style={{
                      width: `${stats.inputBytes > 0 ? Math.min(100, (stats.outputBytes / stats.inputBytes) * 100) : 0}%`,
                    }}
                  />
                </div>
              </div>
